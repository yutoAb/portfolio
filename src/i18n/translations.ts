...
This insight has become the foundation of my career. If the goal is "human-like behavior," the shortest path is to first build the data infrastructure — create the place where massive amounts of data accumulate. That's why I put weight on data engineering and MLOps.`,
  ...
  experience: {
    ...
    tieriv_role: { ja: 'Autoware ワークショップ 2026 / MLOps・AI 系コース', en: 'Autoware Workshop 2026 / MLOps & AI Track' } as T,
    ...
    abeja_desc0: { ja: '内定者インターンとして、卒業後の入社に向けてソフトウェアエンジニア業務に従事', en: 'Joined as a pre-hire intern, building software engineering experience ahead of full-time start after graduation' } as T,
    tieriv_desc0: { ja: '自動運転 OS Autoware・AWSIM を用いた End-to-End 運転モデルの開発', en: 'Developed an end-to-end driving model using the Autoware autonomous-driving OS and AWSIM' } as T,
  ...
  projects: {
    ...
    automationLandscape: { ja: '「衣食住の自動化」を工程 × SAE 準拠 5 段階（L1〜L5）で並べたランドスケープ。第一弾はキッチンで、25 セル中 10 が未充足 gap。家庭向け L5 が事実上 Moley 1 社しか埋まっていない構造などが視覚化される。', en: 'Landscape mapping "the automation of clothing, food, and housing" onto stages × SAE-inspired 5 levels (L1–L5). Volume 1 covers kitchens: 25 cells with 10 unfilled gaps, revealing that consumer-grade L5 is essentially Moley alone.' } as T,
